...
/// <c>canvas</c>, <c>iframe</c>, <c>object</c>, <c>embed</c>. Form controls
/// (<c>input</c>, <c>textarea</c>, <c>select</c>, <c>button</c>) are also
/// replaced per the HTML spec but have internal anonymous structure
/// (placeholder text, dropdown options, etc.) that v1 doesn't render — they
/// stay non-replaced for now and emit no visible UI.
/// </para>
/// <para>
