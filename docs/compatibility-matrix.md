...
| Page-margin boxes (`@top-left`, `@top-center`, `@top-right`, `@bottom-*`, `@left-*`, `@right-*`) | 🧪 | 3 | All 16 boxes: literal + `attr()` + `counter(page)`/`counter(pages)` (incl. `, decimal`) content; per-box style (`font`/`font-*` incl. relative sizes / `color` / `text-align` / `vertical-align` / `background-color` / `border` + per-side `border-<side>` + the `border-width`/`-style`/`-color` box shorthands / `padding` + the border content-inset) + page-context→root inheritance + `@page :first` (Task 21 cycles 3–13). Non-page `counter()` / unsupported counter styles / `counters()` / `string()` / `element()` content, `border-radius`, non-absolute (`%`/`calc()`/`em`) padding, + the §5.3 three-box-per-edge sizing deferred ([deferrals.md](deferrals.md#layout-to-pdf-pipeline)). |
