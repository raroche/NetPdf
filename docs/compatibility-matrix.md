...
| Lengths | ✅ | 4 | `px`/`pt`/unitless, `%` (vs the viewport), `em`/`rem` (vs font-size) — on shape / `<image>` / `<use>` / nested-`<svg>` geometry AND `<text>`/`<tspan>` coordinates (x/dx vs viewport width, y/dy vs height, em/rem vs the run font-size). |
...
| `clip-path` / `mask` references | ✅ | 4 | `clip-path="url(#id)"` unions a `<clipPath>`'s child geometry (clipPathUnits userSpaceOnUse + objectBoundingBox); `mask="url(#id)"` masks (maskContentUnits honored; `mask-type` luminance [default] + alpha, from an attribute or inline `style=`). A url() to a non-clipPath/non-mask target is flagged + left un-clipped/un-masked. `maskUnits` region clip deferred. |
| 2D transforms | ✅ | 4 | `translate`/`scale`/`rotate`/`matrix`/`skewX`/`skewY`. |
| `<text>` / `<tspan>` | ✅ | 4 | Skia text shaping; `text-anchor`, dx/dy, %/em coords, `letter-spacing`/`word-spacing`, per-glyph `rotate`, `dominant-baseline`, font props, gradient fill. Complex-script deferred. |
| `<textPath>` | ✅ | 4 | Glyphs along ANY referenced basic shape (startOffset + text-anchor, tangent rotation, the shape's transform). `textLength`/`lengthAdjust` deferred. |
| `<marker>` references | ✅ | 4 | `marker-start`/`-mid`/`-end` (+ `marker` shorthand, inherited down `<g>`): markerWidth/Height (length units), refX/refY, markerUnits (strokeWidth/userSpaceOnUse), orient (auto/auto-start-reverse/angle), viewBox; vertices incl. the Z close. Curve tangents are chord-approximated. |
| `<filter>` references | 🧪 | 4 | `filter="url(#id)"` — a filter GRAPH → a composed Skia image filter: `feGaussianBlur`/`feOffset`/`feColorMatrix` (matrix/saturate/hueRotate/luminanceToAlpha)/`feDropShadow`/`feFlood`/`feMerge`/`feComposite` (over/in/out/atop/xor/lighter/arithmetic)/`feBlend` (16 modes)/`feMorphology` (erode/dilate)/`feComponentTransfer` (identity/table/discrete/linear/gamma)/`feDisplacementMap`/`feConvolveMatrix`/`feTurbulence` (turbulence/fractalNoise)/`feDiffuseLighting`/`feSpecularLighting` (distant/point/spot lights)/`feImage` (a `data:` raster href placed into the filter region via `preserveAspectRatio`), with named `in`/`in2`/`result` routing (incl. `SourceGraphic`/`SourceAlpha`). Only the primary (reachable) tree contributes; a forward/missing custom `result` ref is treated as unspecified (not flagged); the result is clipped to the filter region (explicit x/y/width/height + `filterUnits` objectBoundingBox/userSpaceOnUse — a zero/negative size renders nothing; else bbox + 10%). Remaining primitives (feTile, feImage element refs), a primitive subregion / `primitiveUnits`, lighting `kernelUnitLength`, an unknown `filterUnits` value, and `BackgroundImage`/`FillPaint` inputs deferred (flagged). |
