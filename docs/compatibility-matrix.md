...
| `<filter>` references | 🧪 | 4 | `filter="url(#id)"` — a filter GRAPH → a composed Skia image filter: `feGaussianBlur`/`feOffset`/`feColorMatrix` (matrix/saturate/hueRotate/luminanceToAlpha)/`feDropShadow`/`feFlood`/`feMerge`/`feComposite` (over/in/out/atop/xor/lighter/arithmetic)/`feBlend` (16 modes)/`feMorphology` (erode/dilate)/`feComponentTransfer` (identity/table/discrete/linear/gamma)/`feDisplacementMap`/`feConvolveMatrix`/`feTurbulence` (turbulence/fractalNoise), with named `in`/`in2`/`result` routing (incl. `SourceGraphic`/`SourceAlpha`). Only the primary (reachable) tree contributes; a forward/missing custom `result` ref is treated as unspecified (not flagged); the composited result is clipped to the DEFAULT filter region (bbox + 10%). Remaining primitives (feImage/feTile/lighting), an EXPLICIT filter region / primitive subregion / `*Units`, and `BackgroundImage`/`FillPaint` inputs deferred (flagged). |
