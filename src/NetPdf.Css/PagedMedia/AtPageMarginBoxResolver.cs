...
internal static class AtPageMarginBoxResolver
{
        ...
        IEnumerable<CssStylesheet> sheets, CssMediaContext media)
    {
        ...
        // so add the SAME {first, non-first} × {left, right} × {blank, non-blank} match-sets per DECLARED
        // page name — else a `chapter:left` / `chapter:blank` / `chapter:first:left` margin-box-only rule
        // would be missed by the "has margin boxes" check + the prefetch (PR #219 review [P1]: was only
        // first-right per name).
        foreach (var name in AtPageRules.DeclaredPageNames(sheets, media))
        {
            foreach (var ctx in AtPageRules.RepresentativeContextsFor(name))
                foreach (var box in ResolveFrom(AtPageRules.EnumeratePageRules(sheets, media, ctx)))
                    (all ??= ImmutableArray.CreateBuilder<ResolvedMarginBox>()).Add(box);
        }
