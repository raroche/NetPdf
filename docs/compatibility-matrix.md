...
| `background-color` | ✅ | 3 | |
| `background-image: url(...)` | 🧪 | 4 | Shipped early (bg-image + margin-box-bg-image cycles): a single `url(...)` on a BODY block — or a PAGE MARGIN BOX — tiles over the border box, gated by `PrintBackgrounds` (incl. the prefetch — no fetch when off), over the `background-color`, under borders/text; partial edge tiles clip at the border box. `background-repeat`/`-size`/`-position` drive the tiling on BOTH (see the row below; margin-box raws are wired post-PR-#167 review). Tilings above the 16-tile per-box loop threshold emit ONE PDF tiling-pattern fill (tiling-patterns cycle, ISO 32000-2 §8.7.3 — O(1) for any count; the old 4096-tile cap is gone). Approximations: the positioning area is the BORDER box (spec: `background-origin: padding-box`); rectangular tiles over a rounded margin-box band. A gradient / multi-layer value surfaces `CSS-BACKGROUND-IMAGE-UNSUPPORTED-001` (the Phase 4 shading-pattern work). |
