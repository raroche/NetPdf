...
- **Speculative measures skip out-of-flow descendants + a nesting budget** (CSS 2.2 §10.3.7, Roland's PR #216 review #4) — an intrinsic min/max-content probe (`NestedContentMeasurer`'s flex / grid pre-measures; `TableLayouter`'s min/max cell passes) used to fully lay out — and recursively re-measure — a box's abspos / fixed descendants and then DROP the fragments (wasted, ~exponential for nested auto-sized boxes). Out-of-flow boxes don't contribute to a box's intrinsic inline width or its §10.6.7 auto block size, so an `isSpeculativeMeasure` flag now gates off the abspos + fixed emission passes during a pure measure — byte-identical (the measured extents are unchanged) while skipping the dropped subtree. Emission callers that FLUSH their buffer (the flex item-content flush, the table cell content) still emit out-of-flow descendants. A thread-local recursion-depth budget (`MaxMeasureNestingDepth` = 96) caps speculative-measure nesting, surfacing `LAYOUT-MEASURE-NESTING-BUDGET-EXCEEDED-001` (new) past it rather than DoS-amplifying.
- **Speculative measures no longer corrupt percentage padding on the shared style** (CSS Sizing §4.1, Roland's PR #216 review #1 — an engine-wide latent bug) — during a max-content probe (1e6), `ResolveUsedPercentPaddingInPlace` rewrote a child's `padding: 10%` to a 100000px length IN PLACE and PERSISTED it onto the SHARED `ComputedStyle`, so the real layout + paint then read the corrupted 100000px inset (the idempotent rewrite skipping re-resolution against the real basis). A speculative measure (the same `isSpeculativeMeasure` flag) now leaves the slot Percentage — it reads as 0 per intrinsic sizing (its basis is indefinite) and stays resolvable against the REAL containing size in the final layout. The four `BlockLayouter` persist sites are gated; byte-identical (the corpus has no %-padded content measured through a 1e6 probe).
- **Fragmentation `break-before/after: left | right | recto | verso` land on the correct page side** (CSS Page L3 §3.4.1, `fragmentation-control-residuals` narrowed) — these forced breaks previously forced a generic page break with no parity. The break's parity (`ForcedPageBreakParityBefore/After` → `BreakOpportunity.ForceParity`, carried out of the layouter via `BlockLayouter.ForcedBreakParityForNextPage`) now reaches the driver, which inserts a blank `@page :blank` page when the resumed content would otherwise land on the wrong-parity page (page 1 is a recto, so LTR recto/right = odd page, verso/left = even; one blank flips it). Byte-identical for documents without these forced breaks. Residual: RTL parity (the driver assumes LTR), the optimizing resolver in production, per-paragraph orphans/widows.
