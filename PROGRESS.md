...
### Next 3-task PR — Flex L1 completion  [active: branch `phase3-flex-l1-completion`; `flex-layouter-features` deferral]
⚠️ **Re-scoped 2026-06-22 after verification** (the handoff's "`flex:1` silently dropped" was STALE — another verify-before-trusting case): the **`flex` shorthand is ALREADY DONE** — `FlexShorthandExpander.TryExpand` ([src/NetPdf.Css/Parser/Preprocessing/FlexShorthandExpander.cs](src/NetPdf.Css/Parser/Preprocessing/FlexShorthandExpander.cs)) handles every §7.4 form (none/auto/`<number>`/two- + three-value/`<basis>`), is wired into `CssPreprocessor.cs:775`, and is tested (`CssPreprocessorShorthandRecoveryTests`). Don't rebuild it; at most add a flex-item integration test (`flex:1` → grow 1 / shrink 1 / basis 0). The REAL remaining flex work (both DEEP — measurement/shaping integration):
1. **`flex-basis` intrinsic keywords in LAYOUT** — `content` / `min-content` / `max-content`. The shorthand + `FlexBasisKind` parse them, and the COLUMN main axis honors content-determined sizing (`IsMainSizeContentDetermined`), but the **ROW main-axis max-content width is deferred** (`FlexLayouter.cs:2777`). Needs intrinsic inline measurement of items.
2. **`align-items: baseline` / `align-self: baseline`** — `FlexLayouter.cs:174` notes baseline cross-axis alignment needs **text-shaping/baseline-metrics integration** (today flex-start/end/center/stretch only). Align items by their first baseline.
3. **Pick a 3rd** (the shorthand being done leaves a slot): e.g. `align-content: baseline`, or audit the remaining `flex-layouter-features` residuals (RTL column cross-axis, margin-box in justify-content free-space). OR pivot the whole PR to **`auto-height-emit-vs-pagination`** (CSS 2.2 → 100%, DEEP, previously reverted) or **Phase 4** visual parity (gradients/shadows/filters/SVG — next major phase).
No conformance bump (Flexbox already 100%); the value is real-world correctness. Tasks 1+2 here are deep measurement integrations — scope carefully.
