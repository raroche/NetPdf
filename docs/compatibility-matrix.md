...
| Page-margin boxes (`@top-left`, `@top-center`, `@top-right`, `@bottom-*`, `@left-*`, `@right-*`) | 🧪 | 3 | All 16 boxes: literal + `attr()` + `counter(page)`/`counter(pages)` (incl. `, decimal`) content; per-box style (`font`/`font-*` incl. relative sizes / `color` / `text-align` / `vertical-align` / `background-color` / `border` + per-side `border-<side>` + the `border-width`/`-style`/`-color` box shorthands / `padding` + the border content-inset) + page-context→root inheritance + `@page :first` + §5.3 shrink-to-fit box sizing (each box's background/border cover its content-sized box) (Task 21 cycles 3–14). Non-page `counter()` / unsupported counter styles / `counters()` / `string()` / `element()` content, `border-radius`, non-absolute (`%`/`calc()`/`em`) padding, + the §5.3 min/max-content distribution (overlap resolution) + explicit margin-box `width`/`height` + overflow clipping deferred ([deferrals.md](deferrals.md#layout-to-pdf-pipeline)). |
