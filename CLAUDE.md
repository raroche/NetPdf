...
2. **Banned dependencies.** No `System.Drawing`, no browser engines, no AGPL/copyleft, no revenue-capped libraries. This is enforced by **process, not a bespoke analyzer**: the dependency-dossier review gate (a new dep needs a reviewed [docs/legal/dependency-dossier.md](docs/legal/dependency-dossier.md) entry before the PackageVersion lands), code review, and the CI vulnerable-dependency scan (`dotnet list package --vulnerable`, see SEC-10). (There is no `NetPdf.BannedAnalyzer` — only the `NetPdf.SourceGen` source generator ships; a `Microsoft.CodeAnalysis.BannedApiAnalyzers` + `BannedSymbols.txt` machine-enforcement is a documented future option.) Approved deps: AngleSharp, AngleSharp.Css, HarfBuzzSharp, SkiaSharp (raster fallback only).
...
**2026-06-18.** Phase 3's layout + pagination ENGINE is feature-complete and **multi-page rendering is live** (driver cycles 0-8, PRs #175-#179): block / inline / flex / grid / table / multicol / absolute layout, the pagination optimizer, paged media (`@page` + the 16 margin boxes + `string()` / `element()` / `counter(page)`), and text shaping + painting all work end-to-end. Inline-level alignment finished through PR #195 (justify on atomic lines + justify-all; inline-block last-line baseline from real metrics; text `vertical-align` line-edge keywords, bounded first cut).

**What's left to finish Phase 3** (do NOT trust older "Next: multi-page driver / design-first" notes in this file's history — that work shipped): W3C conformance MEASUREMENT (the harness is still a smoke stub, so exit criteria 3-6 are unmeasured), a curated feature/polish backlog, perf + memory gate confirmation, and the `0.7.0-beta` release. The remaining work is laid out as ordered **3-task PR groups** in the **roadmap at the top of [PROGRESS.md](PROGRESS.md)** — start there.

For live state run `/phase-status` or read PROGRESS.md; `git log --oneline -1` shows the exact commit.
