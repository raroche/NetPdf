...
public sealed class BlockLayouterTests
{
    ...
    public void Float_border_box_sizing_makes_the_declared_height_the_border_box()
    {
        ...
        // box-sizing too (not just normal-flow blocks): height 100 + 15px top/bottom
        // padding → border-box → the float's block size IS 100 (content 70); content-box
        // → 130. A border-box height SMALLER than the vertical insets floors at them.
        foreach (var (borderBox, declaredHeight, expected) in new[]
        {
            (true, 100.0, 100.0),   // border-box: the declared height IS the border box
            (false, 100.0, 130.0),  // content-box (initial): declared + 30 chrome
            (true, 10.0, 30.0),     // border-box height < 30 insets → floors at the chrome
        })
        {
            var sink = new RecordingFragmentSink();
            var floatStyle = MakeStyle();
            SetLengthPx(floatStyle, PropertyId.Width, 50);
            SetLengthPx(floatStyle, PropertyId.Height, declaredHeight);
            SetLengthPx(floatStyle, PropertyId.PaddingTop, 15);
            SetLengthPx(floatStyle, PropertyId.PaddingBottom, 15);
