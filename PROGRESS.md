...
| 3 | Fragmentainer-aware layout + pagination | 🚧 **Engine done — conformance MET (CSS 2.2 96.7% / Flex + Grid 100% / Frag 90%); `0.7.0-beta` MERGED (#205), git tag pending** |
| 4 | Visual parity (gradients, shadows, filters, SVG) | ⏸️ Not started |
| 5 | Packaging + release | 🔵 Interleaved — layout→PDF wiring done |

**Gates (all green, 2026-06-21):** 7163 unit / 3 skip (+ the 3 perf/memory gates) · 30 LayoutSnapshots · 97 RealDocuments · **W3cConformance (4 per-case-baseline gates; published rates CSS 2.2 97% / Frag 90% / Flex 100% / Grid 100%)** · PaginationGolden · RenderingCorpus · 0-warning Release · AOT/JIT parity · determinism.
...
22. ⏸️ **Tag `0.7.0-beta`** — pending Roland; apply to `af1c210` (#205's merge commit) so the tag captures the signed-off state before post-beta work. Repo is PRIVATE → an INTERNAL milestone tag, NOT a public release, NO NuGet (that's v1.0).

### Post-`0.7.0-beta` — sizing residuals  ✅ DONE (this branch)
