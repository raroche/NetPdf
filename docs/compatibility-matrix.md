...
| `background-color` | ✅ | 3 | |
| `background-image: url(...)` | 🧪 | 4 | First cut shipped early (bg-image cycle): a single `url(...)` on a BODY block tiles over the border box at the intrinsic size (the initial `background-repeat`), gated by `PrintBackgrounds`, over the `background-color`, under borders/text; partial edge tiles clip at the border box. Approximations: the tile phase starts at the BORDER box (spec: `background-origin: padding-box` phase), a 4096-tile per-box cap (`PAINT-BG-IMAGE-TILE-CAP-001`; PDF tiling patterns are the follow-up); `background-position`/`-size`/`-repeat` variants + margin-box backgrounds deferred. A gradient / multi-layer value surfaces `CSS-BACKGROUND-IMAGE-UNSUPPORTED-001` (the Phase 4 shading-pattern work). |
