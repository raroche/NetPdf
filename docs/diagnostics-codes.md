...
| `LAYOUT-MULTICOL-FORCED-OVERFLOW-001` | Warning | A multicol container's in-flow content can't make forward progress through pagination. Cycle 1 shipped Hello World multi-column layout + emitted this diagnostic any time content overflowed N columns (truncating the remainder); cycle 2 ships multi-page multicol via `MulticolContinuation` — a clean multi-page split (content overflows N columns on one page + continues cleanly on the next) is NO LONGER an error. The diagnostic now fires in narrower cases: (a) a `MulticolLayouter` resume page emits zero fragments + its continuation doesn't advance past the entry index (forward-progress safety fallback for the single-oversized-child case); (b) deep-nested multicol containers (`html > body > div.multicol` from a real HTML document, where the multicol sits at recursion depth ≥ 2 inside `EmitBlockSubtreeRecursive`) can't propagate `PageComplete` through cycle 2's single-level recursion-continuation gate — the remainder is truncated + the diagnostic surfaces the loss. Sub-cycle 3+ may generalize multi-level propagation (mirroring Task 13 cycle 2 hardening's `TableContinuation` single-level pattern). The diagnostic message names the column count, the per-column block-size, and the resume child index of the unemitted remainder. See `docs/deferrals.md#multicol-balancing-pagination`. Per Phase 3 Task 14 cycles 1-2. |
