...
| `background-color` | ✅ | 3 | |
| `background-image: url(...)` | 🧪 | 4 | Shipped early (bg-image + margin-box-bg-image cycles): a single `url(...)` on a BODY block — or a PAGE MARGIN BOX (initial repeat/position/size only there) — tiles over the border box, gated by `PrintBackgrounds` (incl. the prefetch — no fetch when off), over the `background-color`, under borders/text; partial edge tiles clip at the border box. `background-repeat`/`-size`/`-position` drive the tiling on body blocks (see the row below). Approximations: the positioning area is the BORDER box (spec: `background-origin: padding-box`), a 4096-tile per-box cap (`PAINT-BG-IMAGE-TILE-CAP-001`; PDF tiling patterns are the follow-up); rectangular tiles over a rounded margin-box band. A gradient / multi-layer value surfaces `CSS-BACKGROUND-IMAGE-UNSUPPORTED-001` (the Phase 4 shading-pattern work). |
