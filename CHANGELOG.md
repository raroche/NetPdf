...
- **Fragmentation `break-before/after: left | right | recto | verso` land on the correct page side** (CSS Page L3 §3.4.1 / §3.6, `fragmentation-control-residuals` narrowed) — these forced breaks previously forced a generic page break with no parity. The break's parity (`ForcedPageBreakParityBefore/After` → `BreakOpportunity.ForceParity`, carried out of the layouter via `BlockLayouter.ForcedBreakParityForNextPage`) now reaches the driver, which inserts a blank `@page :blank` page when the resumed content would otherwise land on the wrong-parity page. `left` / `right` are PHYSICAL (left = even, right = odd page); `recto` / `verso` follow the page progression (LTR recto = right = odd). A forced `left` / `verso` on the FIRST content selects a verso STARTING page (CSS Page §3.6, PR #218 review [P1 #4]) via a parity offset — no leading blank. Byte-identical for documents without these forced breaks. Residual: RTL parity (the driver assumes LTR; `recto` / `verso` would swap), the optimizing resolver in production, per-paragraph orphans/widows.
