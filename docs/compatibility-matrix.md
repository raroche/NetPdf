...
| Page-margin boxes (`@top-left`, `@top-center`, `@top-right`, `@bottom-*`, `@left-*`, `@right-*`) | 🧪 | 3 | All 16 boxes: literal + `attr()` + `counter(page)`/`counter(pages)` (incl. `, decimal`) content; per-box style (`font`/`font-*` incl. relative sizes / `color` / `text-align` / `vertical-align` / `background-color`) + page-context→root inheritance (Task 21 cycles 3–9). Non-page `counter()` / unsupported counter styles / `counters()` / `string()` / `element()` content, margin-box `border`/`padding`, + the §5.3 three-box-per-edge sizing deferred ([deferrals.md](deferrals.md#layout-to-pdf-pipeline)). |
