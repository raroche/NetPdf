...
- **`flex-direction: column` cross-axis honors `direction: rtl`** (CSS Flexbox L1 §3.1 / CSS Box Alignment L3, `flex-layouter-features` narrowed) — a column flex container's cross axis is the inline axis, so `direction: rtl` permutes its cross-start/cross-end (inline-start = right). The per-item `align-items` / `align-self` anchor now flips for a column-rtl container (`isCrossAxisReversed = isWrapReverse ^ isColumnRtl`, fed to `ComputeAlignItemsPlacement`), so `align-items: flex-start` right-anchors and `flex-end` left-anchors — matching browsers — for the common single-line (nowrap) case and per-item within each wrapped line. Residual: the line-to-line STACKING order for a MULTI-LINE column-rtl `wrap` (the line-stacking reversal stays on `wrap-reverse` alone to avoid double-counting). Row and LTR flex are byte-identical (the flip only engages for column + rtl).
