...
**Status:** IN PROGRESS — **Cycles 0–5 done** (nested-container fragmentation + the pipeline driver loop + per-page counters + cross-page running content). Next are cycles 6–7 (`@page :left`/`:right`/`:blank` + named pages). The §8 decisions were taken per the recommended defaults (#3 greedy resolver, #4 child-boundary granularity) — see the Progress log.
...
| **5 ✅** | **Cross-page running content** | DONE (2026-06-15). `MarginContentCollector.CollectPerPage` + an element→page map from the per-page fragment lists → per-page `string(name)` with carry-forward. `element()` cross-page + `start`/`first-except` deferred. | 4 |
