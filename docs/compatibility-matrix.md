...
| `<filter>` references | 🧪 | 4 | `filter="url(#id)"` — a filter GRAPH → a composed Skia image filter: `feGaussianBlur`/`feOffset`/`feColorMatrix` (matrix/saturate/hueRotate/luminanceToAlpha)/`feDropShadow`/`feFlood`/`feMerge`/`feComposite` (over/in/out/atop/xor/arithmetic)/`feBlend` (16 modes), with named `in`/`in2`/`result` routing (incl. `SourceGraphic`/`SourceAlpha`). Other primitives (feImage/feTile/feTurbulence/…), the filter region / primitive subregion / `*Units`, and `BackgroundImage`/`FillPaint` inputs deferred (flagged). |
