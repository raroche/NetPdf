...
| `background-image: linear-gradient()` | ✅ | 4 | PDF native axial shading. Supports `repeating-`, `<angle>`/`to <side\|corner>`, %/length stop positions, double-position stops (§3.4) + color-interpolation hints (§3.4.2, approximated by a synthetic midpoint stop). A translucent stop (per-stop alpha) falls back to a Skia raster (image + `/SMask`) since a native shading is DeviceRGB; opaque stays native. |
| `background-image: radial-gradient()` | ✅ | 4 | PDF native radial shading (ellipse via CTM). Same stop features as linear (double-position, hints, %/length); a translucent stop falls back to a Skia raster (image + `/SMask`); opaque stays native. |
| `background-image: conic-gradient()` | 🧪 | 4 | Skia raster fallback (per-stop alpha + double-position + hints supported via the shared stop pipeline). |
