...
**Status:** IN PROGRESS — **Cycles 0–5b done** (nested-container fragmentation + the pipeline driver loop + per-page counters + cross-page running content — named strings [cycle 5] AND `element()` [cycle 5b]). Next are cycles 6–7 (`@page :left`/`:right`/`:blank` + named pages). The §8 decisions were taken per the recommended defaults (#3 greedy resolver, #4 child-boundary granularity) — see the Progress log.
