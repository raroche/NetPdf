...
| `<text>` / `<tspan>` | ✅ | 4 | Skia text shaping; `text-anchor`, dx/dy, %/em coords, `letter-spacing`/`word-spacing`, per-glyph `rotate` (global index across tspans), `dominant-baseline`, `textLength`/`lengthAdjust` (`spacing` + `spacingAndGlyphs`, single chunk), font props, gradient fill. Complex-script + multi-chunk `textLength` deferred. |
