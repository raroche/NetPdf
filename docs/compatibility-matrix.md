...
| Page-margin boxes (`@top-left`, `@top-center`, `@top-right`, `@bottom-*`, `@left-*`, `@right-*`) | 🧪 | 3 | All 16 boxes: literal + `attr()` + `counter(page)`/`counter(pages)` (incl. `<counter-style>` — decimal / decimal-leading-zero / lower+upper-roman / lower+upper-alpha+latin / lower-greek) content; per-box style (`font`/`font-*` incl. relative sizes / `color` / `text-align` / `vertical-align` / `background-color` / `border` + per-side `border-<side>` + the `border-width`/`-style`/`-color` box shorthands / `padding` + the border content-inset) + page-context→root inheritance + `@page :first` + §5.3 box sizing (shrink-to-fit + explicit `width`/`height` + sibling-overlap distribution with min/max-content flex + content overflow wrapping with per-line alignment) + `string(name)`/`string-set` (incl. the `content()` form — the canonical running header) + `element(name)` (via `position: running()`) content (Task 21 cycles 3–16 + Tasks 22–23). Non-page `counter()` / a non-predefined counter style (e.g. `hebrew`) / `counters()` content, `element()` full block rendering + cross-page running, `border-radius`, non-absolute (`%`/`calc()`/`em`) padding, vertical-edge (height) overflow, `box-sizing` deferred ([deferrals.md](deferrals.md#layout-to-pdf-pipeline)). |
| `string()` / `string-set`, `element()` / `position: running()`, named pages | 🧪 | 3 | FIRST CUT (single page): `string(name [, first \| last])` resolves a `string-set` (literal + `attr()` + `content()` content-lists, one-or-more comma-separated pairs); `content()` is the element's own text, GCPM-normalized as if `white-space: normal` (recovered from AngleSharp via `CssPreprocessor`); `string(name)` defaults to the last assignment (`last`), `first` takes the first; `element(name)` resolves a `position: running()` element's text (removed from normal flow) (Tasks 22–23). DEFERRED: `content(before|after|first-letter|marker)`, `string(name, start \| first-except)` (cross-page), `element()` full block rendering, cross-page "running" persistence, and named-page selectors (multi-page-gated) ([deferrals.md](deferrals.md#layout-to-pdf-pipeline)). |
