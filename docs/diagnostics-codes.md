...
| `LAYOUT-TABLE-FEATURE-UNSUPPORTED-001` | Warning | The table layouter encountered a CSS Tables L3 feature its sub-cycle 1 algorithm doesn't implement (currently: `colspan` / `rowspan` cell merging; sub-cycle 2 will add `table-layout: auto`/`fixed`, `border-collapse`, captions, `<col>` widths, header/footer repetition across pages). The table renders with the feature silently ignored. See `docs/deferrals.md#table-auto-fixed-spans-borders`. Per Phase 3 Task 12 sub-cycle 1. |
