...
internal static class DiagnosticCodes
{
    ...
    /// an unsupported primitive (only <c>feGaussianBlur</c> / <c>feOffset</c> / <c>feColorMatrix</c> /
    /// <c>feDropShadow</c> are modeled — feMerge/feComposite/… aren't) or routing/region attributes
    /// (<c>in</c>/<c>result</c>/subregions), a <c>filter</c> / <c>marker</c> / <c>clip-path</c> / <c>mask</c> /
    /// <c>textPath</c> referencing a target of the wrong kind (or an unresolved id), an unresolved
    /// gradient/pattern ref (<c>fill="url(#…)"</c> → painted transparent, NOT black), an element the renderer
    /// doesn't draw (e.g. <c>&lt;foreignObject&gt;</c> / <c>&lt;switch&gt;</c>), or content truncated by the
    /// depth / element budget. Shapes, paths, text (incl. <c>%</c>/<c>em</c> coordinates, spacing, per-glyph
    /// <c>rotate</c>, <c>dominant-baseline</c>, <c>textPath</c> on any shape), gradients,
    /// <c>&lt;pattern&gt;</c> paint servers, <c>clip-path</c>/<c>mask</c>/<c>filter</c>/<c>marker</c>
    /// references (markers cascade), full <c>preserveAspectRatio</c>, <c>&lt;use&gt;</c>/<c>&lt;symbol&gt;</c>
    /// (with the viewport clip+scale), <c>data:</c> images, stroke dashes, opacity, and nested viewports DO
    /// render. Surfaced once per SVG image.
    /// Severity: <see cref="DiagnosticSeverity.Info"/>.
    /// </summary>
