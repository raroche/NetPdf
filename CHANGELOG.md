...
- **`position: absolute` auto size is true shrink-to-fit / content height** (CSS 2.1 §10.3.7 / §10.6.4, `abspos-cycle-1-explicit-only` narrowed) — an `auto` width/height NOT pinned by both insets previously filled the available extent (CB minus insets/margins/chrome); now it sizes to its content. `EmitOneAbsoluteBox` measures the box's content min-content + max-content INLINE extent (via `NestedContentMeasurer` → `BufferingMeasureSink.ContentInlineExtent`) and `AbsoluteLayouter.ResolvePlacement` clamps the available width into `min(max(min-content, available), max-content)`; the block axis then measures the content height AT the resolved inline width (§10.6.4 ordering). The pinned-both-insets (fill) case stays exact. So `position:absolute; top:10; left:20` around an 80×40 child renders 80×40, not page-sized. Residual: author `min/max-width/height` clamping of the shrink-to-fit result + percentage-padding-against-CB. Boxes with an explicit size, or pinned by both insets, are byte-identical (no measure runs).
