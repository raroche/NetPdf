...
3. ✅ **`orphans` / `widows`** — registered (`Integer`, inherited, default 2) + `ReadOrphansOrDefault`/`ReadWidowsOrDefault`; `PdfRenderPipeline` reads the root box's values into `new BreakResolver(orphans, widows)`. Visible effect awaits line-level splitting (`inline-only-block-line-splitting`); per-paragraph overrides need a per-paragraph resolver value (residual).
