...
| `<text>` / `<tspan>` | ✅ | 4 | Skia text shaping; `text-anchor`, dx/dy, %/em coords, `letter-spacing`/`word-spacing`, per-glyph `rotate` (global index across tspans), `dominant-baseline`, `textLength`/`lengthAdjust` (`spacing` + `spacingAndGlyphs`, single chunk; a whole-text scale for multiple chunks), font props, gradient fill. Complex-script deferred. |
