...
- **CSS Grid `span <custom-ident>` on the START edge with a definite end** (CSS Grid L1 §8.3, `grid-implicit-named-area-and-occurrence-syntax-deferral` narrowed) — `grid-row: span foo / 5` (a span-by-name START with a DEFINITE end) now spans BACKWARD from the resolved end line to the Nth `foo` line before it — the mirror of the already-shipped end-edge span-by-name. The end may be a positive integer, an `<integer> <custom-ident>` occurrence, or a bare `<custom-ident>` (`ResolveDefiniteLine`); the backward count is `ResolveSpanToNamedLineBackward`. A `span <ident>` START with an AUTO / indefinite end still needs the auto-placement span algorithm and falls back to auto. Grids not using this form are byte-identical.
- **Flex `align-items: start` / `end` don't follow `wrap-reverse`** (CSS Box Alignment L3 §6.2, `flex-layouter-features` narrowed) — `start` / `end` are the alignment CONTAINER's writing-mode logical edges, which `wrap-reverse` does NOT permute, unlike the flex-flow-relative `flex-start` / `flex-end`. A second `IsContainerLogical` flag on `ResolvedAlignItems` / `ResolvedAlignSelf` (companion to the `IsSelfRelative` flag) drives a container-direction-only reversal at the per-item anchor, so under `wrap-reverse` `align-items: start` stays at the writing-mode cross-start while `flex-start` permutes to the opposite edge. Coincides with `flex-start` when there's no `wrap-reverse`, so non-`wrap-reverse` flex is byte-identical.
