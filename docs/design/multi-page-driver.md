...
| **5b** | **Cross-page `element()` running content** | Apply the same per-page bucketing + carry-forward to the running-occurrence record (text + own styles / segments / containers). Currently whole-document (correct for a single running element). | 5 |
