...
- Embed logos and icons as **inline `<svg>`** (fully supported — paths, strokes, fills, gradients, `<use>`/`<symbol>`, text) or as an SVG `data:` URI. Inline SVG is the most robust, resolution-independent choice.
